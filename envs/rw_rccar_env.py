...
class RWrccarEnv(Env):
    ...
    def _get_observation(self):
        ...
        vec[vec_keys.index('steer')] = self._ros_msgs['steer'].data
        vec[vec_keys.index('motor')] = self._ros_msgs['motor'].data
    ...
    def _do_backup(self):
        if self._backup_duration > 0:
            ...
            while not rospy.is_shutdown() and (rospy.Time.now() - start_time).to_sec() < self._backup_duration:
                self._set_motor(self._backup_motor)
                rospy.sleep(0.01)
            self._set_motor(0)
    ...
    def ros_is_good(self, print=True):
        ...
        for topic in self._ros_topics_and_types.keys():
            if 'cmd' not in topic and 'collision' not in topic and 'goal' not in topic:
