...
class RccarRealEnv(Env):
    ...
    def _setup_action_spec(self):
        ...
        self.action_spec['speed'] = Box(low=-0.7, high=0.7)
    ...
    def _setup_ros(self, params):
        rospy.init_node('RccarRealEnv', anonymous=True)
        rospy.sleep(1)
    ...
    def _setup_get_ros_topics_and_types(self):
        ...
        return dict([
            ('/usb_cam/image_raw/compressed', sensor_msgs.msg.CompressedImage),
            ('/ackermann_cmd_mux/output', ackermann_msgs.msg.AckermannDriveStamped),
            ('/ackermann_cmd_mux/input/teleop', ackermann_msgs.msg.AckermannDriveStamped),
            ('/collision', std_msgs.msg.Int32)
        ])

    def _setup_ros_publishers(self):
        self._ros_drive_pub = rospy.Publisher(self._ros_namespace + '/ackermann_cmd_mux/input/teleop', ackermann_msgs.msg.AckermannDriveStamped, queue_size=10)
    ...
    def _get_observation(self):
        msg = self._ros_msgs['/usb_cam/image_raw/compressed']
        ...
        vec[vec_keys.index('speed')] = self._ros_msgs['/ackermann_cmd_mux/output'].drive.speed
        vec[vec_keys.index('steer')] = self._ros_msgs['/ackermann_cmd_mux/output'].drive.steering_angle
        vec[vec_keys.index('motor')] = self._ros_msgs['/ackermann_cmd_mux/output'].drive.acceleration
    ...
    def _get_speed(self):
        return self._ros_msgs['/ackermann_cmd_mux/output'].drive.speed
...
if __name__ == '__main__':
    logger.setup(display_name='RccarRealEnv', log_path='/tmp/log.txt', lvl='debug')
    env = RccarRealEnv(params={
        'collision_reward':-1
    })
    test_collision(env)
    # test_reset(env)
